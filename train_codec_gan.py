"""
GAN-based training for Telugu Audio Codec with Discriminators
This is the CORRECT approach used by all production neural codecs
"""
...
# Import codec and discriminator
...
class AudioDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # This ensures ALL samples have consistent amplitude
...
def train_discriminator_step(
    ...
    discriminator.train()
    codec.eval()  # Don't update codec during discriminator training
    
    with autocast():
        # Get real audio
        real_audio = batch.to(next(discriminator.parameters()).device)
        
        # Generate fake audio (detach to not backprop through codec)
        with torch.no_grad():
...
def train_generator_step(
    ...
    """Single generator (codec) training step"""
    codec.train()
    discriminator.eval()  # Don't update discriminator
    ...
    with autocast():
        ...
        # Discriminator evaluation of fake audio
        ...
        # Combined generator loss with balanced weights
        # CRITICAL: Adversarial and feature matching dominate, reconstruction is weak
        total_loss = (
            args.adv_weight * adv_loss +           # Fool discriminator
            args.feat_weight * feat_loss +          # Match features
            args.recon_weight * recon_loss +        # Weak reconstruction
            args.vq_weight * vq_loss                # VQ training
        )
...
def main(args):
    ...
    discriminator = MultiScaleDiscriminator(
        num_scales=args.num_disc_scales
    ).to(device)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train Telugu codec with GAN")
    ...
    parser.add_argument("--num_disc_scales", type=int, default=3)
    ...
    # Loss weights (CRITICAL: Adversarial dominates!)
    parser.add_argument("--adv_weight", type=float, default=1.0, help="Adversarial loss weight")
    parser.add_argument("--feat_weight", type=float, default=2.0, help="Feature matching weight")
    parser.add_argument("--recon_weight", type=float, default=0.1, help="Reconstruction weight (WEAK!)")
    parser.add_argument("--vq_weight", type=float, default=1.0, help="VQ loss weight")
    
    # Logging
    parser.add_argument("--use_wandb", action="store_true")
    parser.add_argument("--experiment_name", type=str, default="telugu_codec_gan")
